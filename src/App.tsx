...
const designs: Design[] = Object.entries(mods).filter(([p])=>!p.toLowerCase().includes("mimis-cozy-logo")).map(([path])=>{
  ...
  return (
    <div style={{background:"#fffaf5",minHeight:"100vh",fontFamily:"system-ui"}}>
      <div style={{background:"white",textAlign:"center",borderBottom:"3px solid #ff69b4",paddingBottom:"14px"}}>
        ...
        <p style={{color:"#a2006d",fontWeight:700,margin:"8px 0"}}>{list.length} Designs • Click → Choose product</p>
        <input placeholder="Search..." value={q} onChange={e=>setQ(e.target.value)} style={{padding:"10px 20px",borderRadius:"20px",border:"1px solid #ffb6d9",width:"320px"}}/>
      </div>
      ...
      {sel&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:"16px"}} onClick={()=>setSel(null)}><div style={{background:"white",borderRadius:"20px",maxWidth:"520px",width:"100%",overflow:"hidden"}} onClick={e=>e.stopPropagation()}><img src={sel.image} style={{width:"100%",maxHeight:"340px",objectFit:"contain",background:"#fef2f8"}}/><div style={{padding:"16px"}}><h3 style={{margin:"0 0 10px"}}>{sel.title}</h3><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"12px"}}>{PRODS.map(p=>(<button key={p.name} onClick={()=>setSelProd(p)} style={{padding:"10px",borderRadius:"10px",border: selProd.name===p.name? '2px solid #a2006d':'1px solid #eee',background: selProd.name===p.name? '#fef2f8':'white',fontWeight:700,fontSize:"12px"}}>{p.emoji} {p.name} {p.price}</button>))}</div><button onClick={create} disabled={creating} style={{width:"100%",padding:"12px",background:"#a2006d",color:"white",borderRadius:"10px",fontWeight:800,border:"none"}}>{creating? 'CREATING...': `CREATE ${selProd.name}`}</button><button onClick={()=>setSel(null)} style={{width:"100%",marginTop:"8px",padding:"8px",border:"none",background:"#eee",borderRadius:"8px"}}>Close</button></div></div></div>)}
    </div>
  );
}
